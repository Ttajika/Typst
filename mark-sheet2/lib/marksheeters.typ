...
#let mark-shape(num, fill:white,col:black, tag:none) = {
...
#let mark_ans(answer, //正答番号
  ...
  for i in range(choice){
    mark-shape(
      ...
      col:col,
      tag: if tag == none {none} else {(..tag, opt:i)}
      )
...
#let mark_answer(answers, //回答の配列
                ...
                choice:choice, //選択肢数
                breaks:(), //大問の区切り。(q: 先頭設問番号, label: 表示名 | none) の辞書の配列。整数の配列も可（線のみ）
                ) = {
  let n = answers.len() //解答がある設問の数を取得
  let bks = breaks.map(b => if type(b) == int { (q: b, label: none) } else { b }) //整数指定も辞書に揃える
  let question = () //生成する配列を入れる空配列を作成
  for i in range(N) {
    let hit = bks.find(b => b.q == i + 1)
    if hit != none {
      if i > 0 {
        //区切り線。hline だと太い線の端が縦罫線の外縁まで延長されて両端 0.5pt はみ出すため、
        //罫線ではなく高さ 1pt の帯をセルの中身として描く。上下の通常罫線と合わさって約 2pt の線に見え、
        //端は通常罫線と同じ位置（縦罫線の中心）で止まる。
        question.push(table.cell(colspan: 2, inset: 0pt, stroke: (left: none, right: none),
          rect(width: 100%, height: 1pt, fill: black, stroke: none)))
      }
      if hit.label != none {
        question.push(table.cell(colspan: 2, align: left + horizon, inset: (x: 4pt, y: 1.5pt),
          text(size: 0.55em, weight: 700, hit.label))) //大問名の帯
      }
    }
    question.push(text(font:mark-font, size:10pt, weight:mark-weight)[#numbering(numbering-style,i+1)]) //設問番号を表示
    question.push(mark_ans( if i < n {answers.at(i)} else {},choice:choice, tag:(kind:"q", idx:i+1))) //マーク欄を表示
  }
  return question //配列を出力，これを後で表に出力する
}

//FormScanner認識用の４隅の丸
#let maru = box[#context [#metadata((
    page: here().page(),
    x: here().position().x.pt(),
    y: here().position().y.pt(),
  )) <fiducial>]#circle(stroke:4pt, radius: 8pt)]
      ...
      for j in range(num){
        IDs.push(
          mark-shape(
            ...
            tag:(kind:"id", idx:j+1, opt:i, ch:response.at(i))
            )
          )  
      }  
  }
...
#let marked-sheet(///塗りつぶす番号の配列
                  ...
                  ///大問の先頭となる設問番号の配列（例: (14, 27)）。解答欄にその行の上の太線として反映される。
                  ///auto なら mondai の位置から自動で決まる
                  ///-> array|auto
                  breaks:(),
                  ///区切りに大問名（mondai の name＋番号）の帯を表示するか。breaks: auto のときのみ有効
                  ///-> bool
                  labels:false,
                  ) = {
  ...
  context {
    //auto の場合は mondai が記録した開始位置を使う
    let bk = if breaks == auto {
      let rec = state("ms-breaks", ()).final().dedup().filter(b => b.q <= N)
      if labels { rec } else { rec.filter(b => b.q > 1).map(b => (q: b.q, label: none)) }
    } else { breaks }
    let n-columns = calc.ceil((N + bk.filter(b => b.label != none).len() * 0.5) / 25) //ラベル帯の分も列数に加味
    align(center,block(width:95%)[ //blockで幅を抑え，認識用二重丸マークでマーク領域をうまく囲めるようにする
      ...
      #table(
        ...
        ..mark_answer(answers, numbering-style:numbering-style, N,choice:choice, breaks:bk))]]) //解答マーク欄
  }
}
...
#let setmon(
  ...
  context{
    ...
    if note != none {
      state("ms-kaisetsu", ()).update(s => s + ((num: num, note: note),)) //解説を記録
    }
...
#let Qref(label,
          ...
          kuran:"空欄") = {
    context{
      let kind = counter("toi"+label+"kind").final().at(0)
      if kind == 1 {kasen} else if kind == 2 {kuran}
        numbering(numbering-style,counter("toi"+label).final().at(0))
      } 
}
...
#let choicelist(
  ...
  box(mark-shape(text(size:mark-size,)[#numbering("1",start)]), baseline: -.1em)
  [#separator]
 box(mark-shape(text(size:mark-size)[#numbering("1",start + n - 1)]), baseline: -.1em)
}
...
#let mark-number(
  n
    ...
    box(mark-shape(text(size:mark-size,)[#n]), baseline: -.1em)
}
...
#let mondai(body,
            name:"問題",
            ///false にすると answer-breaks: auto のとき解答用紙にこの大問の区切り線を引かない
            ///-> bool
            sheet-break:true,
            ///answer-labels: true のとき解答用紙の区切りに表示する名前。auto なら「問題 n」
            ///-> auto|none|content
            sheet-label:auto,
            ) = {
  counter("mondai").step()
  if sheet-break {
    context {
      let k = counter("kuran").get().at(0) //この大問の直前までの設問数
      let mn = counter("mondai").get().at(0)
      let lb = if sheet-label == auto { [#name #numbering("1", mn)] } else { sheet-label }
      state("ms-breaks", ()).update(s => s + ((q: k + 1, label: lb),)) //先頭の設問番号と表示名を記録
    }
  }
...
#let setmonR(
          ...
          /// -> array,
          other:(),
          ///ランダム化のキー．ここを変更することで，選択肢の並び替えパターンを変更できる．
          ///-> int
          randomize_key:42, 
          ///列数
          ///-> int
...
// ===========================================================================
//  用紙情報ページ
//  組版し終えた自分の座標を query で読み、圧縮した記述子を文字として印刷する。
//  読み取り側は PDF からこのテキストを拾えばよく、typst query を外から
//  呼ぶ必要がない（typst.app だけで完結できる）。
//  正答と配点が印刷されるので、このページは学生に配らないこと。
// ===========================================================================
#let layout-page() = {
  pagebreak()
  set page(flipped: false, margin: 1.5cm, header: none, footer: none)
  set text(size: 5.5pt, hyphenate: false, font: ("DejaVu Sans Mono", "harano aji mincho"))
  context {
    let ms = query(<markpos>).map(m => m.value)
    let fd = query(<fiducial>).map(m => m.value)
    let ak = query(<answerkey>).first().value
    let pg = calc.max(..ms.map(m => m.page))
    let last = ms.filter(m => m.page == pg)
    let r(v) = calc.round(v, digits: 2)

    let qrows = ()
    for i in range(1, ak.N + 1) {
      let c = last.filter(m => m.kind == "q" and m.idx == i and m.opt == 0)
      if c.len() > 0 { qrows.push((r(c.first().x), r(c.first().y))) }
    }
    let q1 = last.filter(m => m.kind == "q" and m.idx == 1)
    let dx = if q1.len() > 1 { r(q1.at(1).x - q1.at(0).x) } else { 15.0 }

    let payload = json.encode((
      N: ak.N, choice: ak.choice, IDn: ak.ID_num, resp: ak.response.join(""),
      dx: dx, total: ak.total,
      fid: fd.filter(m => m.page == pg).map(m => (r(m.x + 8), r(m.y + 8))),
      q: qrows,
      idx: last.filter(m => m.kind == "id" and m.opt == 0).map(m => r(m.x)),
      idy: last.filter(m => m.kind == "id" and m.idx == 1).map(m => r(m.y)),
      ans: ak.answers, pt: ak.points, pat: ak.patterns,
    ), pretty: false)

    // 1行ずつ通し番号を付けて印刷する。PDF のテキスト抽出は行の順序を
    // 保証しないため、長い文字列をそのまま流すと復元時に順番が狂う。
    // 番号を付けておけば読み取り側で並べ直せる。
    text(size: 8pt)[この頁は用紙情報です。正答が含まれるため学生に配らないでください。]
    linebreak()
    let n = 72
    let k = 0
    while k * n < payload.len() {
      let part = payload.slice(k * n, calc.min((k + 1) * n, payload.len()))
      [MS|#str(k)|#part|]
      linebreak()
      k += 1
    }
...
#let mark-rei(
  ...
  let mw = mark-width * scale-f
  let mh = mark-height * scale-f
  let base(fill: white, body) = ellipse(
    width: mw, height: mh, fill: fill, stroke: .5pt,
    align(center + horizon, body))
  let rei(label, mark) = box(inset: (x: .45em))[
    #align(center)[
      #box(width: mw * 1.5, height: mh * 1.15, align(center + horizon, mark)) \
      #text(size: .75em)[#label]]]

  box(stroke: .75pt, inset: (x: 10pt, y: 7pt), radius: 2pt)[
    #set text(size: 10pt)
    #box(baseline: -1.2em)[*マーク例*]
    #h(1em)
    #rei([*良い例*], base(fill: black)[])
    #h(.6em)
    #box(baseline: -.5em, height: 2.4em, line(angle: 90deg, length: 100%, stroke: .5pt))
    #h(.6em)
    #box(baseline: -1.2em)[悪い例：]
    #rei([レ点], base[#place(dx:-0.6em,text(weight: 700, size: .9em * scale-f)[✓])])
    #rei([点だけ], base[#circle(radius: 1.2pt * scale-f, fill: black)])
    #rei([薄い], base(fill: luma(205))[])
    #rei([はみ出し], {
      box(width: mw * 1.5, height: mh * 1.15)[
        #place(center + horizon, dx: -mw * .15, base[])
        #place(center + horizon, dx: mw * .18, dy: -mh * .18,
          ellipse(width: mw * .85, height: mh * .85, fill: black, stroke: none))]
    })
  ]
}


/// setmon の note: に書いた解説を、設問番号・正答・配点つきで一覧表示する。
/// 返却用の模範解答ページなどに置いて使う。
/// -> content
#let kaisetsu-list(
  ///設問番号のナンバリング
  ...
  let items = state("ms-kaisetsu", ()).final()
  if items.len() == 0 { return }
  for e in items {
    let head = [*問 #numbering(numbering-style, e.num)*]
    if show-answer {
      let ans = counter("kuran-" + str(e.num)).final().at(0)
      let pt = counter("kuran-" + str(e.num) + "point").final().at(0)
      head += [　（正答 #ans ・ #pt 点）]
    }
    block(above: 1em, below: .4em, head)
    block(inset: (left: 1.5em), e.note)
  }
}
