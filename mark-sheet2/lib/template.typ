...
#let project(
    ...
    layout-info: true, //用紙情報ページ（正答を含む）を末尾に出すか
    answer-breaks: (), //解答用紙の大問区切り。auto なら mondai の位置から自動、配列（例: (14, 27)）で手動指定
    answer-labels: false, //区切りに大問名の帯を表示するか（answer-breaks: auto のときのみ有効）
    show-kaisetsu: false, //setmon の note: を集めた解説一覧を正答一覧の後に出すか
    marksheet-ans:false,
    marksheet-pat:false,
    marksheet-pts:false,
    body //本文
    ...
    context[
      ...
      #metadata((
        N: N, choice: choice, ID_num: ID_num, response: response,
        answers: answers, points: points, patterns: patterns, total: total-points,
      )) <answerkey>
      #pagebreak() 
      //解答及び配点一覧
      ...
      //正答マークシートを作成
      #if marksheet-ans {marked-sheet(
        breaks: answer-breaks, labels: answer-labels,
        answers:answers,
  ...
  if show-kaisetsu {
    heading(numbering: none, outlined: false)[解説]
    kaisetsu-list()
  }
  if layout-info { layout-page() }
}
